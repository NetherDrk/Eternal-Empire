...
namespace UnityEngine.UI
{
  ...
  public class InputField : Selectable, IEventSystemHandler, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IUpdateSelectedHandler, ISubmitHandler, ICanvasElement
  {
    ...
    public string text
    {
      get
      {
        if (this.m_Keyboard != null && this.m_Keyboard.active && (!this.InPlaceEditing() && (UnityEngine.Object) EventSystem.current.currentSelectedGameObject == (UnityEngine.Object) this.gameObject))
    ...
    /// 
    /// <para>
    /// Optional graphic to show when the InputField text is empty.
    ...
    /// 
    /// <para>
    /// Current InputField selection head.
    /// </para>
    /// 
    /// </summary>
    ...
    private IEnumerator CaretBlink()
    {
      ...
      return (IEnumerator) new InputField.\u003CCaretBlink\u003Ec__Iterator3()
      {
        \u003C\u003Ef__this = this
      };
    }
    ...
    private int GetUnclampedCharacterLineFromPosition(Vector2 pos, TextGenerator generator)
    {
      ...
      {
        float num3 = generator.lines[index].topY;
        float num4 = num3 - (float) generator.lines[index].height;
        if ((double) num1 > (double) num3)
        {
          float num5 = num3 - num2;
          if ((double) num1 > (double) num3 - 0.5 * (double) num5)
            return index - 1;
          return index;
        }
    ...
    protected int GetCharacterIndexFromPosition(Vector2 pos)
    {
      ...
      int num = cachedTextGenerator.lines[lineFromPosition].startCharIdx;
      int lineEndPosition = InputField.GetLineEndPosition(cachedTextGenerator, lineFromPosition);
      for (int index = num; index < lineEndPosition && index < cachedTextGenerator.characterCountVisible; ++index)
      {
        UICharInfo uiCharInfo = cachedTextGenerator.characters[index];
        Vector2 vector2 = uiCharInfo.cursorPos / this.m_TextComponent.pixelsPerUnit;
        if ((double) (pos.x - vector2.x) < (double) (vector2.x + uiCharInfo.charWidth / this.m_TextComponent.pixelsPerUnit - pos.x))
          return index;
      }
    ...
    {
      // ISSUE: object of a compiler-generated type is created
      return (IEnumerator) new InputField.\u003CMouseDragOutsideRect\u003Ec__Iterator4()
      {
        eventData = eventData,
        \u003C\u0024\u003EeventData = eventData,
        \u003C\u003Ef__this = this
...
label_26:
    ...
    private int LineUpCharacterPosition(int originalPos, bool goToFirstChar)
    {
      ...
      int num = this.cachedInputTextGenerator.lines[index1].startCharIdx - 1;
      for (int index2 = this.cachedInputTextGenerator.lines[index1 - 1].startCharIdx; index2 < num; ++index2)
      {
        if ((double) this.cachedInputTextGenerator.characters[index2].cursorPos.x >= (double) uiCharInfo.cursorPos.x)
          return index2;
      }
      return num;
    }
    ...
    private int LineDownCharacterPosition(int originalPos, bool goToLastChar)
    {
      ...
      for (int index = this.cachedInputTextGenerator.lines[num + 1].startCharIdx; index < lineEndPosition; ++index)
      {
        if ((double) this.cachedInputTextGenerator.characters[index].cursorPos.x >= (double) uiCharInfo.cursorPos.x)
          return index;
      }
      return lineEndPosition;
    }
    ...
    /// 
    /// <para>
    /// Convenience function to make functionality to send the SubmitEvent easier.
    /// </para>
    /// 
    /// </summary>
    ...
    private void SetDrawRangeToContainCaretPosition(int caretPos)
    {
      ...
      if (this.multiLine)
      {
        ...
        int line1 = this.DetermineCharacterLine(caretPos, this.cachedInputTextGenerator);
        int num1 = (int) size.y;
        if (this.m_DrawEnd <= caretPos)
        {
          this.m_DrawEnd = InputField.GetLineEndPosition(this.cachedInputTextGenerator, line1);
          for (int line2 = line1; line2 >= 0 && line2 < lines.Count; --line2)
          {
            num1 -= lines[line2].height;
            if (num1 < 0)
              break;
            this.m_DrawStart = InputField.GetLineStartPosition(this.cachedInputTextGenerator, line2);
          }
        }
        else
        {
          if (this.m_DrawStart > caretPos)
            this.m_DrawStart = InputField.GetLineStartPosition(this.cachedInputTextGenerator, line1);
          int line2 = this.DetermineCharacterLine(this.m_DrawStart, this.cachedInputTextGenerator);
          int line3 = line2;
          this.m_DrawEnd = InputField.GetLineEndPosition(this.cachedInputTextGenerator, line3);
          int num2 = num1 - lines[line3].height;
          while (true)
          {
            while (line3 >= lines.Count - 1)
            {
              if (line2 <= 0)
                return;
              --line2;
              if (num2 < lines[line2].height)
                return;
              this.m_DrawStart = InputField.GetLineStartPosition(this.cachedInputTextGenerator, line2);
              num2 -= lines[line2].height;
            }
            ++line3;
            if (num2 >= lines[line3].height)
            {
              this.m_DrawEnd = InputField.GetLineEndPosition(this.cachedInputTextGenerator, line3);
              num2 -= lines[line3].height;
            }
            else
              break;
          }
    ...
    private void GenerateCaret(VertexHelper vbo, Vector2 roundingOffset)
    {
      ...
      float num1 = (float) this.m_CaretWidth;
      ...
      {
        UICharInfo uiCharInfo = cachedTextGenerator.characters[charPos];
      ...
      int index1 = this.DetermineCharacterLine(charPos, cachedTextGenerator);
      zero.y = cachedTextGenerator.lines[index1].topY / this.m_TextComponent.pixelsPerUnit;
      float num2 = (float) cachedTextGenerator.lines[index1].height / this.m_TextComponent.pixelsPerUnit;
      for (int index2 = 0; index2 < this.m_CursorVerts.Length; ++index2)
        this.m_CursorVerts[index2].color = (Color32) this.caretColor;
      ...
      if (roundingOffset != Vector2.zero)
      {
        for (int index2 = 0; index2 < this.m_CursorVerts.Length; ++index2)
        {
          UIVertex uiVertex = this.m_CursorVerts[index2];
      ...
      if (Screen.fullScreen && targetDisplay < Display.displays.Length)
        num3 = Display.displays[targetDisplay].renderingHeight;
      zero.y = (float) num3 - zero.y;
    ...
    private void GenerateHightlight(VertexHelper vbo, Vector2 roundingOffset)
    {
      ...
      int lineEndPosition = InputField.GetLineEndPosition(cachedTextGenerator, line);
      UIVertex v = UIVertex.simpleVert;
      v.uv0 = Vector2.zero;
      v.color = (Color32) this.selectionColor;
      for (int index = charPos; index <= num3 && index < cachedTextGenerator.characterCountVisible; ++index)
      {
        if (index + 1 == lineEndPosition || index == num3)
        {
          UICharInfo uiCharInfo1 = cachedTextGenerator.characters[charPos];
          UICharInfo uiCharInfo2 = cachedTextGenerator.characters[index];
          Vector2 vector2_1 = new Vector2(uiCharInfo1.cursorPos.x / this.m_TextComponent.pixelsPerUnit, cachedTextGenerator.lines[line].topY / this.m_TextComponent.pixelsPerUnit);
          Vector2 vector2_2 = new Vector2((uiCharInfo2.cursorPos.x + uiCharInfo2.charWidth) / this.m_TextComponent.pixelsPerUnit, vector2_1.y - (float) cachedTextGenerator.lines[line].height / this.m_TextComponent.pixelsPerUnit);
          ...
          v.position = new Vector3(vector2_1.x, vector2_2.y, 0.0f) + (Vector3) roundingOffset;
          vbo.AddVert(v);
          v.position = new Vector3(vector2_2.x, vector2_2.y, 0.0f) + (Vector3) roundingOffset;
          vbo.AddVert(v);
          v.position = new Vector3(vector2_2.x, vector2_1.y, 0.0f) + (Vector3) roundingOffset;
          vbo.AddVert(v);
          v.position = new Vector3(vector2_1.x, vector2_1.y, 0.0f) + (Vector3) roundingOffset;
          vbo.AddVert(v);
          vbo.AddTriangle(currentVertCount, currentVertCount + 1, currentVertCount + 2);
          vbo.AddTriangle(currentVertCount + 2, currentVertCount + 3, currentVertCount);
          charPos = index + 1;
          ++line;
          lineEndPosition = InputField.GetLineEndPosition(cachedTextGenerator, line);
        }
      }
    }
    ...
    /// 
    /// </summary>
    /// <param name="text">The whole text string to validate.</param><param name="pos">The position at which the current character is being inserted.</param><param name="ch">The character that is being inserted.</param>
    /// <returns>
    /// 
    /// <para>
    ...
    /// 
    /// </summary>
    /// <param name="text"/><param name="charIndex"/><param name="addedChar"/>
