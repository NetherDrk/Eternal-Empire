  ...
  public abstract class Graphic : UIBehaviour, ICanvasElement
  {
    ...
    public virtual Material materialForRendering
    {
      get
      {
        ...
        this.GetComponents(typeof (IMaterialModifier), list);
        Material baseMaterial = this.material;
        for (int index = 0; index < list.Count; ++index)
          baseMaterial = (list[index] as IMaterialModifier).GetModifiedMaterial(baseMaterial);
    ...
    private void CacheCanvas()
    {
      ...
      if (list.Count > 0)
      {
        for (int index = 0; index < list.Count; ++index)
        {
          if (list[index].isActiveAndEnabled)
          {
            this.m_Canvas = list[index];
            break;
          }
        }
      }
    ...
    private void DoMeshGeneration()
    {
      ...
      List<Component> list = ListPool<Component>.Get();
      this.GetComponents(typeof (IMeshModifier), list);
      for (int index = 0; index < list.Count; ++index)
        ((IMeshModifier) list[index]).ModifyMesh(Graphic.s_VertexHelper);
      ListPool<Component>.Release(list);
    ...
    private void DoLegacyMeshGeneration()
    {
      ...
      this.GetComponents(typeof (IMeshModifier), list);
      for (int index = 0; index < list.Count; ++index)
        ((IMeshModifier) list[index]).ModifyMesh(Graphic.workerMesh);
      ListPool<Component>.Release(list);
      this.canvasRenderer.SetMesh(Graphic.workerMesh);
    }
    ...
    /// 
    /// </summary>
    /// <param name="m">Mesh to populate with UI data.</param><param name="vh"/>
    ...
    {
      foreach (MonoBehaviour monoBehaviour in this.gameObject.GetComponents<MonoBehaviour>())
      {
        ...
        if (method != null)
          method.Invoke((object) monoBehaviour, (object[]) null);
      }
    }
    ...
    /// 
    /// </summary>
    /// <param name="sp">Screen point.</param><param name="eventCamera">Camera.</param>
    /// <returns>
    /// 
    /// <para>
    ...
    public virtual bool Raycast(Vector2 sp, Camera eventCamera)
    {
      ...
      for (; (UnityEngine.Object) transform != (UnityEngine.Object) null; transform = transform.parent)
      {
        ...
        for (int index = 0; index < list.Count; ++index)
        {
          ICanvasRaycastFilter canvasRaycastFilter = list[index] as ICanvasRaycastFilter;
          if (canvasRaycastFilter != null)
          {
            bool flag2 = true;
            CanvasGroup canvasGroup = list[index] as CanvasGroup;
    ...
    /// 
    /// </summary>
    /// <param name="targetColor">Target color.</param><param name="duration">Tween duration.</param><param name="ignoreTimeScale">Should ignore Time.scale?</param><param name="useAlpha">Should also Tween the alpha channel?</param>
    ...
    /// 
    /// </summary>
    /// <param name="alpha">Target alpha.</param><param name="duration">Duration of the tween in seconds.</param><param name="ignoreTimeScale">Should ignore Time.scale?</param>
