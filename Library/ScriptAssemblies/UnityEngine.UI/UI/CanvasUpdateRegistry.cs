...
namespace UnityEngine.UI
{
  ...
  public class CanvasUpdateRegistry
  {
    ...
    private void CleanInvalidItems()
    {
      ...
      {
        ICanvasElement canvasElement = this.m_LayoutRebuildQueue[index];
      ...
      {
        ICanvasElement canvasElement = this.m_GraphicRebuildQueue[index];
        if (canvasElement == null)
    ...
    private void PerformUpdate()
    {
      ...
      for (int index1 = 0; index1 <= 2; ++index1)
      {
        ...
        {
          ICanvasElement element = CanvasUpdateRegistry.instance.m_LayoutRebuildQueue[index2];
          try
          {
            if (this.ObjectValidForUpdate(element))
              element.Rebuild((CanvasUpdate) index1);
          }
          catch (Exception ex)
          {
            Debug.LogException(ex, (UnityEngine.Object) element.transform);
          }
        }
      }
      ...
      for (int index1 = 3; index1 < 5; ++index1)
      {
        for (int index2 = 0; index2 < CanvasUpdateRegistry.instance.m_GraphicRebuildQueue.Count; ++index2)
        {
          try
          {
            ICanvasElement element = CanvasUpdateRegistry.instance.m_GraphicRebuildQueue[index2];
            if (this.ObjectValidForUpdate(element))
              element.Rebuild((CanvasUpdate) index1);
          }
          catch (Exception ex)
    ...
    private bool InternalRegisterCanvasElementForLayoutRebuild(ICanvasElement element)
    {
      ...
      Debug.LogError((object) string.Format("Trying to add {0} for layout rebuild while we are already inside a layout rebuild loop. This is not supported.", (object) element));
      return false;
    }
