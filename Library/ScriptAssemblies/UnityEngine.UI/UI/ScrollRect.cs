  ...
  public class ScrollRect : UIBehaviour, IEventSystemHandler, IBeginDragHandler, IInitializePotentialDragHandler, IDragHandler, IEndDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutController, ILayoutGroup
  {
    ...
    public virtual void OnDrag(PointerEventData eventData)
    {
      ...
      if (eventData.button != PointerEventData.InputButton.Left || !this.IsActive() || !RectTransformUtility.ScreenPointToLocalPointInRectangle(this.viewRect, eventData.position, eventData.pressEventCamera, out localPoint))
        return;
      this.UpdateBounds();
      Vector2 vector2_1 = this.m_ContentStartPosition + localPoint - this.m_PointerStartLocalCursor;
      Vector2 vector2_2 = this.CalculateOffset(vector2_1 - this.m_Content.anchoredPosition);
      Vector2 position = vector2_1 + vector2_2;
      if (this.m_MovementType == ScrollRect.MovementType.Elastic)
      {
        if ((double) vector2_2.x != 0.0)
          position.x = position.x - ScrollRect.RubberDelta(vector2_2.x, this.m_ViewBounds.size.x);
        if ((double) vector2_2.y != 0.0)
          position.y = position.y - ScrollRect.RubberDelta(vector2_2.y, this.m_ViewBounds.size.y);
      }
      this.SetContentAnchoredPosition(position);
    }
    ...
    private Vector2 CalculateOffset(Vector2 delta)
    {
      ...
      if (this.m_MovementType == ScrollRect.MovementType.Unrestricted)
        return zero;
      Vector2 vector2_1 = (Vector2) this.m_ContentBounds.min;
      Vector2 vector2_2 = (Vector2) this.m_ContentBounds.max;
      if (this.m_Horizontal)
      {
        vector2_1.x += delta.x;
        vector2_2.x += delta.x;
        if ((double) vector2_1.x > (double) this.m_ViewBounds.min.x)
          zero.x = this.m_ViewBounds.min.x - vector2_1.x;
        else if ((double) vector2_2.x < (double) this.m_ViewBounds.max.x)
          zero.x = this.m_ViewBounds.max.x - vector2_2.x;
      }
      if (this.m_Vertical)
      {
        vector2_1.y += delta.y;
        vector2_2.y += delta.y;
        if ((double) vector2_2.y < (double) this.m_ViewBounds.max.y)
          zero.y = this.m_ViewBounds.max.y - vector2_2.y;
        else if ((double) vector2_1.y > (double) this.m_ViewBounds.min.y)
          zero.y = this.m_ViewBounds.min.y - vector2_1.y;
      }
      return zero;
    }
